...
EXPOSE 8050
HEALTHCHECK --interval=30s --timeout=3s CMD python -c "import os,urllib.request; p=os.environ.get('API_PORT','5000'); urllib.request.urlopen(f'http://127.0.0.1:{p}/health').read()" || exit 1
CMD ["bash","-c","gunicorn --bind 0.0.0.0:${API_PORT:-5000} --workers ${WORKERS:-2} --timeout ${TIMEOUT:-120} wsgi:server & npm run start --prefix web -- --hostname 0.0.0.0 --port ${PORT:-8050}"]

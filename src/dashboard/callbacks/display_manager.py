...
def register_display_callbacks(app):
    # Add a store to manage the show_trades state
    app.layout.children.insert(0, dcc.Store(id='show-trades-store-1', data=False))

    # Single callback for updating section 2 content and toggling trades
    @app.callback(
        [
            Output('tab-1-section-2-content', 'children'),
            Output('tab-2-section-2-content', 'children'),
            Output('show-trades-store-1', 'data')
        ],
        [
            Input('data-store-1', 'data'),
            Input('data-store-2', 'data'),
            Input('tabs', 'value'),
            Input('display-trades-button-1', 'n_clicks')
        ],
        [
            State('show-trades-store-1', 'data')
        ],
        prevent_initial_call=True
    )
    def update_section_2(data_store_1, data_store_2, active_tab, display_trades_clicks, show_trades):
        ...
        # Toggle show_trades state when Display Trades button is clicked
        if trigger_id == 'display-trades-button-1' and display_trades_clicks > 0:
            new_show_trades = not show_trades
            # Only update content_1 if data_store_1 exists and future_df is available
            if data_store_1:
                ...
                if not future_df.empty:
                    fig = get_candlestick_plot(ticket, future_df, performance_df, show_trades=new_show_trades)
            ...
            return [content_1, content_2, new_show_trades]

        # Handle other triggers (data-store-1, data-store-2, tabs)
        if trigger_id == 'tabs':
            return [default_content, content_2, show_trades]
            ...
            if future_df.empty:
                ...
                fig = get_candlestick_plot(ticket, future_df, performance_df, show_trades=show_trades)
                future_plot = dcc.Graph(
                    id='candlestick-plot',
                    figure=fig,
                    responsive=True,
            ...
            return [content_1, content_2, show_trades]
